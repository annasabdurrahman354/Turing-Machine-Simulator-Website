...
<script>
export default {
  name: 'Modulo Multitrack', // NAMA DIUBAH
  props: {
  ...
  methods: {
    ...
    start(){ // GAUSAH DIGANTI
        this.onStart = true
        alert("Mulai")
        this.q2(this.tapes1, this.tapes2, this.before, this.active)
    },

    q2(tapes1, tapes2, before, active){           // STATE
        ...
        if(tapes1[active].symbol == '0' && tapes2[active].symbol == 'b' ){  //IF INPUTAN
            ...
            setTimeout(() => this.q2(tapes1, tapes2, active, active +1), this.delay);  // +1 ke kanan -1 ke kiri
        }
