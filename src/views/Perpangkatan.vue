...
<div class="px-4 flex flex-col justify-center items-center bg-gray-200 h-full w-full">
    ...
    <div class="w-2/6 min-h-min bg-white shadow-md flex flex-col justify-center items-center px-4 py-4 rounded-lg gap-4">
        <div class="w-full max-h-16 flex justify-center items-center rounded-lg gap-4">
            ...
            <input v-model="b" :disabled="onGenerate" class="form-control" type="number" min="0" name="b" id="b" required>
        </div>
        <div v-if="!onGenerate">
    ...
    <div class="bg-white rounded rounded-lg shadow-md py-3 px-6 mt-4">
  ...
  methods: {
    ...
    q0(tapes, before, active){          //ganti nama state
        ...
        switch (tapes[active].symbol) { //gak usah edit
            case 'x':                                                           //ganti apa aja case symbol di state saaat ini
                tapes[active].symbol = "b"                                      //symbol input diubah jadi apa
                setTimeout(() => this.q4(tapes, active, active+1), this.delay); //q1 ganti nama state, active+1 kalo ke kanan, -1 ke kiri
                break;                                  
            case 'y':                                                           
    ...
    q11(tapes,before, active){
        ...
        switch (tapes[active].symbol) {
            case 'b':
                ...
                setTimeout(() => this.q33(tapes, active, active+1), this.delay);
                break;
        }
    },
    ...
    q19(tapes,before, active){
        ...
        switch (tapes[active].symbol) {
            ...
            case 'b':
                tapes[active].symbol = "b"
                setTimeout(() => this.q25(tapes, active, active+1), this.delay);
                break;
        }
    },
    ...
    q21(tapes,before, active){
        ...
        switch (tapes[active].symbol) {
            case 'x':
                tapes[active].symbol = "x"
                setTimeout(() => this.q22(tapes, active, active+1), this.delay);
                break;
            case 'b':
                tapes[active].symbol = "b"
                setTimeout(() => this.q21(tapes, active, active+1), this.delay);
    ...
    q22(tapes,before, active){
        ...
        switch (tapes[active].symbol) {
            ...
            case 'b':
                tapes[active].symbol = "0"
                this.tapes.push({symbol:"b", current:false})
                setTimeout(() => this.q23(tapes, active, active-1), this.delay);
                break;
        }
    },
    q23(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
            ...
            case 'x':
                tapes[active].symbol = "x"
                setTimeout(() => this.q24(tapes, active, active-1), this.delay);
                break;
        }
    },
    q24(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
    ...
    q25(tapes,before, active){
        ...
        switch (tapes[active].symbol) {
            ...
            case '1':
                tapes[active].symbol = "1"
                setTimeout(() => this.q26(tapes, active, active-1), this.delay);
                break;
        }
    },
    q26(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
            case '0':
                tapes[active].symbol = "z"
                setTimeout(() => this.q27(tapes, active, active+1), this.delay);
                break;
        }
    },
    q27(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
            case '1':
                tapes[active].symbol = "1"
                setTimeout(() => this.q28(tapes, active, active+1), this.delay);
                break;
        }
    },
    q28(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
            ...
            case '0':
                tapes[active].symbol = "b"
                setTimeout(() => this.q29(tapes, active, active-1), this.delay);
                break;
            case 'x':
                tapes[active].symbol = "x"
                setTimeout(() => this.q41(tapes, active, active-1), this.delay);
                break;
        }
    },
    q29(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
            ...
            case '1':
                tapes[active].symbol = "1"
                setTimeout(() => this.q30(tapes, active, active-1), this.delay);
                break;
        }
    },
    q30(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
            ...
            case '0':
                tapes[active].symbol = "z"
                setTimeout(() => this.q31(tapes, active, active+1), this.delay);
                break;
        }
    },
    q31(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
            ...
            case '1':
                tapes[active].symbol = "1"
                setTimeout(() => this.q32(tapes, active, active+1), this.delay);
                break;
        }
    },
    q32(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
    ...
    q33(tapes,before, active){
        ...
        switch (tapes[active].symbol) {
            ...
            case 'b':
                tapes[active].symbol = "b"
                setTimeout(() => this.q34(tapes, active, active-1), this.delay);
                break;
        }
    },
    q34(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
            ...
            case '0':
                tapes[active].symbol = "z"
                setTimeout(() => this.q35(tapes, active, active+1), this.delay);
                break;
        }
    },
    q35(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
            ...
            case 'b':
                tapes[active].symbol = "0"
                this.tapes.push({symbol:"b", current:false})
                setTimeout(() => this.q36(tapes, active, active-1), this.delay);
                break;
        }
    },
    q36(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
    ...
    q37(tapes,before, active){
        ...
        switch (tapes[active].symbol) {
            case 'z':
                tapes[active].symbol = "0"
                setTimeout(() => this.q37(tapes, active, active+1), this.delay);
                break;
            case '1':
                tapes[active].symbol = "1"
                setTimeout(() => this.q38(tapes, active, active+1), this.delay);
                break;
        }
    },
    q38(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
            ...
            case 'x':
                tapes[active].symbol = "x"
                setTimeout(() => this.q39(tapes, active, active+1), this.delay);
                break;
        }
    },
    q39(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
            ...
            case 'x':
                tapes[active].symbol = "x"
                setTimeout(() => this.q40(tapes, active, active-1), this.delay);
                break;
        }
    },
    q40(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
    ...
    q41(tapes,before, active){
        ...
        switch (tapes[active].symbol) {
            ...
            case '1':
                tapes[active].symbol = "b"
                setTimeout(() => this.q42(tapes, active, active-1), this.delay);
                break;
        }
    },
    q42(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
            case 'z':
                tapes[active].symbol = "b"
                setTimeout(() => this.q43(tapes, active, active-1), this.delay);
                break;
        }
    },
    q43(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
            ...
            case 'x':
                tapes[active].symbol = "x"
                setTimeout(() => this.q44(tapes, active, active+1), this.delay);
                break;
        }
    },
    q44(tapes,before, active){
        tapes[before].current = false; 
        this.before = active
        tapes[active].current = true; 
        switch (tapes[active].symbol) {
            ...
            case 'b':
                tapes[active].symbol = "b"
                setTimeout(() => this.q45(tapes, active, active+1), this.delay);
                break;
        }
    },
